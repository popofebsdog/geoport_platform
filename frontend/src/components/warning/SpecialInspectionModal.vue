          ...
          <div class="space-y-4">
            ...
            <div>
              ...
              <select
                ...
                class="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 focus:ring-2 focus:ring-blue-500 focus:border-transparent disabled:bg-gray-100 dark:disabled:bg-gray-800 disabled:cursor-not-allowed"
              >
                <option :value="null">請選擇日期</option>
                <option v-for="d in availableDays" :key="d" :value="d">{{ d }}日</option>
              </select>
            </div>
            ...
            <div>
              <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                特殊事件名稱 <span class="text-red-500">*</span>
              </label>
              <input
                v-model="formData.eventName"
                type="text"
                placeholder="請輸入特殊事件名稱..."
                class="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 focus:ring-2 focus:ring-blue-500 focus:border-transparent"
              />
            </div>
...
export default {
  ...
  computed: {
    ...
    availableDays() {
      if (!this.formData.month || !this.formData.year) return []
    ...
    isVisible(newVal) {
      if (newVal) {
        if (this.editingRecord) {
          ...
          // 特殊事件名稱可能在 eventName 或 header 字段中
          const eventName = data.eventName || data.header || ''
  ...
  methods: {
    ...
    async handleSubmit() {
      ...
      if (!this.isFormValid) {
        await showAlert('請填寫完整資訊', '提示', this.isDarkMode)
        return
      }
      ...
      try {
        const inspectionDate = `${this.formData.year}-${String(this.formData.month).padStart(2, '0')}-${String(this.formData.day).padStart(2, '0')}`
        ...
        console.error('保存特別巡查記錄失敗:', error)
